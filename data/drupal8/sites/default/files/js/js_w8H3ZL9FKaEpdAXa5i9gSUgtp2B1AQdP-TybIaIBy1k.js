...
(function ($, Drupal, drupalSettings) {
   ...
   * Unchanging vars
   */
  var path_prefix = drupalSettings.path.baseUrl + drupalSettings.path.pathPrefix + 'citi_bike/';

  console.log(drupalSettings.path.pathPrefix)
  ...
  $.fn.citiBike = function(options) {
     ...
     * Elements
     */
    var $blockContainer = $this.closest('.block.block-citi-bike');
    var $stationContainer = $('.citi-bike-container', $this);
    var $stationHeader = $('.citi-bike-stations', $this);
    var $stationLoader = $('.fb-loading-icon', $this);
    ...
    // Fade out the stations
    function fadeContainerOut() {
      $stationContainer.fadeOut(500);
    }
    ...
    /**
     * Load stations and set up tiles
     * @param after
     */
    var loadStations = function(after) {
      var suffix = '';
      ...
      $.ajax({
        url: path_prefix + blockID + '/stations' + suffix,
        dataType: 'json',
        success: function(response) {

          if (response.data != null && response.data.content != null) {

            if (firstBatch && stationInitialized) {
              scrollToAlbum();
            ...
            if ($stationContainer.hasClass('stations-covers')) {
              $stationContainer.append(response.data.content);
              $stationContainer.fadeIn(500);
            }
          }
        }
      });
     ...
     */
    var initCitiBikeStations = function() {
      loading = false;
      firstBatch = true;

      $stationContainer.html('');
      $stationContainer.removeClass('citi-bike-photos');
      $stationContainer.addClass('citi-bike-covers');
      $stationHeader.html('');
